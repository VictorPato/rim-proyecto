...
int main(int argc, char* argv[]) {
    if( argc != 4 ){
        std::cout << " Usage: ./find_neighbours_from_folder <descriptors_folder> <test_folder> <amount of neighbours>" << std::endl;
        return -1; 
    }
    ...
    std::vector<std::string> tests = listar_archivos(test_folder);
    int i = 0;
    int percent = 0;
    int prev_percent = -1;
    ...
    for(std::string image_name : tests){
        i = 0;
        percent = 0;
        prev_percent = -1;
        total_size = descriptor_vector.size();
        std::cout << "Testing for " << image_name << std::endl;
        ...
        std::cout << "Best "<< k << " neighbours: " << std::endl;
